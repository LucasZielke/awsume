...
def get_powershell_file() -> str:
    (powershell_file, _) = subprocess.Popen(
        ["powershell", "Write-Host $profile"], stdout=subprocess.PIPE, shell=True
    ).communicate()
    if powershell_file:
        powershell_file = (
            str(powershell_file.decode("ascii")).replace("\r\n", "").replace("\n", "")
        )
...
def parse_args(argv: sys.argv) -> argparse.Namespace:
    ...
    if not args.shell and (args.autocomplete_file or args.alias_file):
        parser.error(
            "Cannot specify autocomplete file or alias file when not specifying shell"
        )
