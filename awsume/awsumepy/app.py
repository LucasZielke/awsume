...
from .lib.profile import aggregate_profiles, credentials_to_profile, is_mutable_profile
...
class Awsume:
    ...
    def get_saml_credentials(self, args: argparse.Namespace, profiles: dict) -> dict:
        ...
        if not roles:
            raise exceptions.SAMLAssertionMissingRoleError(
                "No roles found in the saml assertion"
            )
        role_arn = None
        principal_arn = None
        ...
        if len(roles) > 1:
            if args.role_arn and args.principal_arn:
                ...
                if self.config.get("fuzzy-match"):
                    ...
                    safe_print(f"Closest match: {choice}")
                else:
                    if principal_plus_role_arn not in roles:
                        raise exceptions.SAMLRoleNotFoundError(
                            args.principal_arn, args.role_arn
                        )
                    else:
                ...
                if principal_plus_profile_role_arn in roles:
                    ...
                    raise exceptions.SAMLRoleNotFoundError(
                        principal_arn, profile_role_arn
                    )
                safe_print(f"Match: {choice}")
            else:
        ...
        for arn in choice.split(","):
            ...
            if "saml-provider" in arn:
                principal_arn = arn
        safe_print(
            f"Assuming role: {principal_arn},{role_arn}",
    ...
    def get_credentials(self, args: argparse.Namespace, profiles: dict) -> dict:
        ...
        try:
            if (
                not args.auto_refresh and args.json
                ...
                credentials = self.plugin_manager.hook.get_credentials(
                    config=self.config, arguments=args, profiles=profiles
                )
                ...
                if args.auto_refresh:
                    ...
                    if self.config.get("is_interactive"):
                        logger.debug(
                            "Interactive execution, killing existing autoawsume processes"
                        )
                        kill_autoawsume()
        except exceptions.ProfileNotFoundError as e:
            self.plugin_manager.hook.catch_profile_not_found_exception(
                config=self.config, arguments=args, profiles=profiles, error=e
            )
            raise
        except exceptions.InvalidProfileError as e:
            self.plugin_manager.hook.catch_invalid_profile_exception(
                config=self.config, arguments=args, profiles=profiles, error=e
            )
            raise
        except exceptions.UserAuthenticationError as e:
            self.plugin_manager.hook.catch_user_authentication_error(
                config=self.config, arguments=args, profiles=profiles, error=e
            )
            raise
        except exceptions.RoleAuthenticationError as e:
            self.plugin_manager.hook.catch_role_authentication_error(
                config=self.config, arguments=args, profiles=profiles, error=e
            )
            raise
    ...
    def export_data(
        ...
        if arguments.output_profile and not arguments.auto_refresh:
            ...
            if "Expiration" in credentials:
                awsumed_profile["expiration"] = credentials["Expiration"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            add_section(
                arguments.output_profile, awsumed_profile, credentials_file, True
            )
    ...
    def run(self, system_arguments: list):
        try:
            ...
            if args.auto_refresh:
                return self.export_data(
                    ...
                    [
                        str(
                            args.output_profile
                            or f"autoawsume-{args.target_profile_name}"
                        ),
            ...
            logger.debug("", exc_info=True)
            if self.is_interactive:
                safe_print(f"Awsume error: {e}", color=colorama.Fore.RED)
                sys.exit(1)
            else:
                raise
