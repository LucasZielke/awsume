...
def assume_role(
    ...
    try:
        ...
        role_session["Expiration"] = role_session["Expiration"].astimezone(
            dateutil.tz.tzlocal()
        )
    ...
    if cache_lib.valid_cache_session(cache_session) and not ignore_cache:
        ...
        try:
            kwargs = {
                ...
                "TokenCode": None
                if not mfa_serial
                else (mfa_token or profile_lib.get_mfa_token()),
            }
            if duration_seconds:
                kwargs["DurationSeconds"] = duration_seconds
            user_session = user_sts_client.get_session_token(**kwargs).get(
                "Credentials"
            )
            user_session["Expiration"] = user_session["Expiration"].astimezone(
                dateutil.tz.tzlocal()
            )
...
def assume_role_with_saml(
    ...
    try:
        ...
        if role_duration:
            kwargs["DurationSeconds"] = int(role_duration)
        role_session = role_sts_client.assume_role_with_saml(**kwargs).get(
            "Credentials"
        )
        role_session["Expiration"] = role_session["Expiration"].astimezone(
            dateutil.tz.tzlocal()
    ...
    safe_print(
        "Role credentials will expire {}".format(
            profile_lib.parse_time(role_session["Expiration"])
        ),
        colorama.Fore.GREEN,
    )
    return role_session
