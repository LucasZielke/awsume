...
def validate_profile(
    ...
    if arguments.output_profile and not is_mutable_profile(
        profiles, arguments.output_profile
    ):
        raise exceptions.ImmutableProfileError(
            arguments.output_profile, "not awsume-managed"
        )
    if not profile:
    ...
    if "role_arn" in profile:
        if profile.get("credential_source") and profile.get("source_profile"):
            raise exceptions.InvalidProfileError(
                target_profile_name,
                message="credential_source and source_profile are mutually exclusive profile options",
            )
        if (
            not profile.get("credential_source")
            and not profile.get("credential_process")
            and not profile.get("source_profile")
            and not profile.get("principal_arn")
        ):
            raise exceptions.InvalidProfileError(
                target_profile_name,
                message="role profiles must contain one of credential_source or source_profile or credential_process",
            )
        if profile.get("credential_source") not in VALID_CREDENTIAL_SOURCES:
            raise exceptions.InvalidProfileError(
                target_profile_name,
                message="unsupported awsume credential_source profile option: {}".format(
    ...
    if user_profile:
        ...
        if missing_keys:
            raise exceptions.InvalidProfileError(
                user_profile_name,
                message="Missing keys {}, or credential_source, or credential_process".format(
                    ", ".join(missing_keys)
                ),
            )
...
def get_role_chain(
    ...
    logger.debug(f"Getting role chain for [{target_profile_name}]")
    target_profile = profiles.get(target_profile_name)

    if (
        not target_profile
        or not target_profile.get("role_arn")
        or get_role_duration(config, arguments, target_profile)
    ):
...
def get_region(
    ...
    if (
        arguments.role_arn
        and arguments.source_profile
        and profiles.get(arguments.source_profile, {}).get("region")
    ):
...
def get_role_duration(
    config: dict, arguments: argparse.Namespace, target_profile: dict
):
...
def format_aws_profiles(
    profiles: dict, get_extra_data: bool
    ...
    for name in sorted_profiles:
        ...
        if "auto-refresh-" not in name:
            ...
            profile_region = str(profile.get("region")) or str(
                profile.get("sso_region")
            )
...
def list_profile_data(
    profiles: dict, get_extra_data: bool, config: dict
):  # pragma: no cover
...
def get_account_id(profile: dict, call_aws: bool = False) -> str:
    ...
    if (
        call_aws
        and profile.get("aws_access_key_id")
        and profile.get("aws_secret_access_key")
    ):
...
def get_profile_name(
    config: dict, profiles: dict, target_profile_name: str, log=True
) -> str:
    ...
    if matched_prefix_profile:
        if log:
            safe_print(
                "Using profile " + matched_prefix_profile, color=colorama.Fore.YELLOW
            )
    ...
    if matched_contains_profile:
        if log:
            safe_print(
                "Using profile " + matched_contains_profile, color=colorama.Fore.YELLOW
            )
    ...
    if matched_levenshtein_profile:
        if log:
            safe_print(
                "Using profile " + matched_levenshtein_profile,
...
def match_contains(profile_names: list, profile_name: str) -> str:
    ...
    matches = {
        profile: longest_contains(profile_name, profile) for profile in profile_names
    }
    biggest_match = max(matches.values())
    result = [k for k in matches if matches[k] == biggest_match]
    if len(result) == 1:
        return result[0]
    return None
...
def match_levenshtein(profile_names: list, profile_name: str) -> str:
    if not Levenshtein:
        ...
        return None
    matches = {
        profile: Levenshtein.distance(profile_name, profile)
        for profile in profile_names
    }
    closest_match = min(matches.values())
    result = [k for k in matches if matches[k] == closest_match]
    if len(result) == 1:
        return result[0]
    return None
