...
def parse_assertion(assertion: str) -> list:
    if not xmltodict:
        raise ValidationException(
            message="SAML option not installed, try installing awsume[saml]"
        )
    ...
    if response.get("saml2p:Response") is not None:
        attributes = (
            response.get("saml2p:Response", {})
            .get("saml2:Assertion", {})
            .get("saml2:AttributeStatement", {})
            .get("saml2:Attribute", {})
        )
        attribute_value_key = "saml2:AttributeValue"
    else:
        attributes = (
            response.get("samlp:Response", {})
            .get("saml:Assertion", {})
            .get("saml:AttributeStatement", {})
            .get("saml:Attribute", {})
        )
    ...
    for attribute in [
        _
        for _ in attributes
        if _.get("@Name", "") == "https://aws.amazon.com/SAML/Attributes/Role"
    ]:
