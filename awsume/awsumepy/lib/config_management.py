...
def migrate_to_xdg_base_directories() -> None:
    ...
    if constants.IS_USING_XDG_CONFIG_HOME and os.path.isfile(
        constants.AWSUME_CONFIG_LEGACY_PATH
    ):
        # handle migration to XDG base directory: config file
        move(constants.AWSUME_CONFIG_LEGACY_PATH, constants.AWSUME_CONFIG)
    if constants.IS_USING_XDG_CACHE_HOME and os.path.isdir(
        constants.AWSUME_CACHE_DIR_LEGACY_PATH
    ):
        # handle migration to XDG base directory: cache dir
        move(constants.AWSUME_CACHE_DIR_LEGACY_PATH, constants.AWSUME_CACHE_DIR)
    if constants.IS_USING_XDG_DATA_HOME and os.path.isdir(
        constants.AWSUME_CACHE_DIR_LEGACY_PATH
    ):
        # handle migration to XDG base directory: data dir
        move(constants.AWSUME_DIR_LEGACY_PATH, constants.AWSUME_DIR)
    if constants.IS_USING_XDG_STATE_HOME and os.path.isdir(
        constants.AWSUME_LOG_DIR_LEGACY_PATH
    ):
        # handle migration to XDG base directory: log dir
        move(constants.AWSUME_LOG_DIR_LEGACY_PATH, constants.AWSUME_LOG_DIR)
    if (
        constants.IS_USING_XDG_CONFIG_HOME
        and constants.IS_USING_XDG_DATA_HOME
        and constants.IS_USING_XDG_CACHE_HOME
        and constants.IS_USING_XDG_STATE_HOME
        and os.path.isdir(constants.AWSUME_DIR_LEGACY_PATH)
    ):
...
def handle_config(operations: list):
    ...
    if not len(operations):
        raise exceptions.ConfigOperationException(
            "Must supply a config management operand"
        )
    ...
    if operations[0].lower() == "get":
        ...
        logger.debug(f"Getting {operations[1]}")
    ...
    if operations[0].lower() == "list":
        if len(operations) != 1:
            raise exceptions.ConfigOperationException(
                'No operands are valid for operation "list"'
            )
    ...
    if operations[0].lower() == "set":
        if len(operations) < 3:
            raise exceptions.ConfigOperationException(
                f"Must supply value to set {operations[1]} to"
            )
        logger.debug(f"Setting {operations[1]} to {operations[2]}")
    ...
    if operations[0].lower() in ["reset"]:
        ...
        if default_value is None:
            raise exceptions.ConfigOperationException(
                f"Key does not have a default: {operations[1]}",
                colorama.Fore.YELLOW,
            )
        config = update_dict_parts(config, operations[1], default_value)
        safe_print(
            f"Reset key {operations[1]} to {default_value}",
            colorama.Fore.YELLOW,
        )
    ...
    if operations[0].lower() in ["clear"]:
        ...
        if deleted:
            safe_print(f"Deleted key {operations[1]}", colorama.Fore.YELLOW)
