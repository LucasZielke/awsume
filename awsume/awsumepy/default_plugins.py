...
from .lib.profile import VALID_CREDENTIAL_SOURCES, get_profile_name, get_role_chain
...
def add_arguments(config: dict, parser: argparse.ArgumentParser):
    ...
    assume_role_method.add_argument(
        "--with-saml",
        action="store_true",
        dest="with_saml",
...
def post_add_arguments(
    ...
    if arguments.auto_refresh:
        if arguments.role_arn:
            raise exceptions.ValidationException(
                "Cannot use autoawsume with a given role_arn"
            )
        if arguments.json:
    ...
    if arguments.role_arn and not arguments.role_arn.startswith("arn:"):
        ...
        if len(parts) == 2:
            ...
            parser.error(
                '--role-arn must be a valid role arn or follow the format "<partition>:<account_id>:<role_name>"'
            )
        if not account_id.isnumeric() or len(account_id) != 12:
            parser.error(
                "--role-arn account id must be valid numeric account id of length 12"
            )
        arguments.role_arn = f"arn:{partition}:iam::{account_id}:role/{role_name}"
    ...
    if arguments.principal_arn and not arguments.principal_arn.startswith("arn:"):
        ...
        if len(parts) == 2:
            ...
            parser.error(
                '--principal-arn must be a valid role arn or follow the format "<partition>:<account_id>:<provider_name>"'
            )
    ...
    if not arguments.profile_name:
        if arguments.role_arn:
            ...
            logger.debug(
                'No profile name passed, target profile name will be "default"'
            )
    ...
    if arguments.session_tags:
        ...
        for tag in arguments.session_tags.split(","):
            kv = tag.split("=")
            try:
                tags.append({"Key": kv[0], "Value": kv[1] or ""})
            except IndexError:
...
def collect_aws_profiles(
    ...
    logger.debug(f"Collected {len(profiles)} profiles")
    return profiles


@hookimpl(tryfirst=True)
def post_collect_aws_profiles(
    ...
    if arguments.list_profiles:
        ...
        profiles = profile_lib.list_profile_data(
            profiles, arguments.list_profiles == "more", config
        )
        raise exceptions.EarlyExit(data={"profiles": profiles})


def assume_role_from_cli(config: dict, arguments: dict, profiles: dict):
    region = profile_lib.get_region(
        profiles, arguments, config, ignore_config=True, ignore_default=True
    )
    logger.info("Using role_arn from the CLI")
    role_duration = arguments.role_duration or int(config.get("role-duration", 0))
    session_name = (
        arguments.session_name or config.get("role-session-name") or "awsume-cli-role"
    )
    ...
    if not arguments.source_profile:
        ...
        if role_duration:
            ...
            if mfa_serial:
                ...
                logger.debug(
                    "Using custom role duration for role that needs mfa_serial, skipping get-session-token call"
                )
            ...
            if mfa_serial:
                ...
                source_session = aws_lib.get_session_token(
                    ...
                    duration_seconds=config.get("debug", {}).get(
...
def get_assume_role_credentials_mfa_required(
    config: dict,
    arguments: argparse.Namespace,
    profiles: dict,
    target_profile: dict,
    ...
    if source_profile:
        ...
        "credential_source" in target_profile
        and target_profile["credential_source"] in VALID_CREDENTIAL_SOURCES
    ):
        logger.debug("Using current environment to assume role")
        source_session = {}

    if arguments.auto_refresh and (
        os.environ.get("AWS_PROFILE", "").startswith("autoawsume-")
        or profiles.get(os.getenv("AWS_PROFILE"), {}).get("autoawsume")
    ):
...
def get_assume_role_credentials_mfa_required_large_custom_duration(
    config: dict,
    arguments: argparse.Namespace,
    profiles: dict,
    target_profile: dict,
    ...
    if arguments.auto_refresh and role_duration > 3600:
        raise exceptions.ValidationException(
            "Cannot use autoawsume with custom role duration of more than 1 hour"
        )
    logger.debug(
        "Skipping the get_session_token call, temp creds cannot be used for custom role duration"
    )
...
def get_credentials_no_mfa(
    config: dict, arguments: argparse.Namespace, profiles: dict, target_profile: dict
):
...
def get_credentials_from_credential_source(
    config: dict,
    arguments: argparse.Namespace,
    profiles: dict,
    target_profile: dict,
...
def get_credentials_from_credential_process(
    config: dict,
    arguments: argparse.Namespace,
    profiles: dict,
    target_profile: dict,
    ...
    credential_process_target_and_arguments = (
        get_credentials_process_target_and_arguments(target_profile)
    )
    ...
    if result.returncode:
        logger.debug(f"No creds returned from credential_process: {result}")
        message = (
    ...
    if not creds:
        raise exceptions.NoCredentialsError(
            "credential_process did not return a valid session"
        )
    return_session = {}
    ...
    return_session["Region"] = region
    logger.debug(f"credential_process session: {return_session}")
    return return_session
...
def get_credentials_process_target_and_arguments(target_profile: dict):
    ...
    if target is None:
        logger.debug(
            f"Unable to find credentials target from provided process string: {credential_process}"
        )
        raise exceptions.ValidationException(
            f"credential_process target not found: {target}"
        )
    if not target.is_file():
        logger.debug(
            f"{credential_process} is not a file. Derived target is {target!s}"
        )
        raise exceptions.ValidationException(
            f"credential_process target is not a file: {target}"
        )
    return [str(target), *parts[1:]]


def get_session_token_credentials(
    config: dict,
    arguments: argparse.Namespace,
    profiles: dict,
    target_profile: dict,
...
def get_credentials_handler(
    ...
    logger.info(f"Getting credentials: {profile_name}")
    ...
    if arguments.role_arn:
        ...
        if "credential_process" in target_profile:
            session = get_credentials_from_credential_process(
                config, arguments, profiles, target_profile, profile_name
            )
            ...
            if (
                mfa_serial and not credentials
            ):  # if using specific credentials, no mfa needed
                if (
                    role_duration > 3600
                ):  # cannot use temp creds with custom role duration more than an hour
                    role_session = (
                        get_assume_role_credentials_mfa_required_large_custom_duration(
                            config,
                            arguments,
                            profiles,
                            target_profile,
                    ...
                    user_session, role_session = (
                        get_assume_role_credentials_mfa_required(
                            config,
                            arguments,
                            profiles,
                            target_profile,
                ...
                role_session = get_assume_role_credentials(
                    config,
                    arguments,
                    profiles,
                    target_profile,
            ...
            if mfa_serial:
                user_session = get_session_token_credentials(
                    config, arguments, profiles, target_profile, profile_name
                )
            elif "credential_source" in target_profile:
                user_session = get_credentials_from_credential_source(
                    config, arguments, profiles, target_profile, profile_name
                )
            else:
                user_session = get_credentials_no_mfa(
                    config, arguments, profiles, target_profile
                )

    if config.get("is_interactive"):
        if user_session and user_session.get("Expiration"):
            safe_print(
                "Session token will expire at {}".format(
                    profile_lib.parse_time(user_session["Expiration"])
                ),
                colorama.Fore.GREEN,
            )
        if role_session and role_session.get("Expiration"):
            safe_print(
                "[{}] Role credentials will expire {}".format(
                    profile_name, profile_lib.parse_time(role_session["Expiration"])
                ),
                colorama.Fore.GREEN,
            )
...
def get_credentials(
    ...
    if arguments.role_arn:
        ...
        target_profile_name = get_profile_name(
    ...
    logger.debug(f"Role chain: {role_chain}")
    credentials = None
    for profile_name in role_chain:
        credentials = get_credentials_handler(
            config=config,
            arguments=arguments,
            profiles=profiles,
