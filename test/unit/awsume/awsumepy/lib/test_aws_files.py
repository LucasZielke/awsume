...
from awsume.awsumepy.lib import aws_files, constants


@patch.dict(
    "os.environ", {"AWS_CONFIG_FILE": "", "AWS_SHARED_CREDENTIALS_FILE": ""}, clear=True
)
...
def test_get_aws_files_args():
    args = argparse.Namespace(
        config_file="my/config/file", credentials_file="my/credentials/file"
    )
    config = {}
...
@patch.dict(
    "os.environ",
    {
        "AWS_CONFIG_FILE": "my/config/file",
        "AWS_SHARED_CREDENTIALS_FILE": "my/credentials/file",
    },
    clear=True,
)
...
def test_add_section(ConfigParser: MagicMock, open: MagicMock):
    ...
    aws_files.add_section(
        "section-name", {"key": "value", "key2": "value2"}, "file-name", overwrite=True
    )
...
@patch("builtins.open")
@patch("configparser.ConfigParser")
def test_add_section_no_overwrite(
    ConfigParser: MagicMock, open: MagicMock, safe_print: MagicMock
):
    parser = MagicMock()
    ConfigParser.return_value = parser
    parser.has_section.return_value = True
    ...
    aws_files.add_section(
        "section-name", {"key": "value", "key2": "value2"}, "file-name", overwrite=False
    )
...
@patch("builtins.open")
@patch("configparser.ConfigParser")
def test_add_section_new_section(
    ConfigParser: MagicMock, open: MagicMock, safe_print: MagicMock
):
    parser = MagicMock()
    ConfigParser.return_value = parser
    ...
    aws_files.add_section(
        "section-name", {"key": "value", "key2": "value2"}, "file-name"
    )
