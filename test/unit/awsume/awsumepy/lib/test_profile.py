...
from awsume.awsumepy.lib.exceptions import (
    InvalidProfileError,
...
def test_get_region():
    arguments = argparse.Namespace(
        region=None, role_arn=None, source_profile=None, target_profile_name="myrole"
    )
    config = {}
    profiles = {
        "default": {
    ...
    assert (
        profile.get_region(profiles, arguments, config) == profiles["myrole"]["region"]
    )
...
def test_get_region_cli_role_arn_and_source_profile():
    ...
    assert (
        profile.get_region(profiles, arguments, config) == profiles["myuser"]["region"]
    )
...
def test_get_region_default_region():
    ...
    assert (
        profile.get_region(profiles, arguments, config) == profiles["default"]["region"]
    )
...
def test_get_account_id_role_arn(get_account_id: MagicMock):
    assert (
        profile.get_account_id(
            {"role_arn": "arn:aws:iam::123456789012:role/S3Access"}, False
        )
        == "123456789012"
    )
...
def test_get_account_id_mfa_serial(get_account_id: MagicMock):
    assert (
        profile.get_account_id(
            {"mfa_serial": "arn:aws:iam::123456789012:mfa/Bob"}, False
        )
        == "123456789012"
    )
    get_account_id.assert_not_called()
