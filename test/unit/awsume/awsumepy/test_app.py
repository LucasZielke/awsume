...
from awsume.awsumepy import app, hookspec
from awsume.awsumepy.lib import aws, exceptions
...
def test_app_init(
    init: MagicMock, get_plugin_manager: MagicMock, load_config: MagicMock
):
    obj = app.Awsume()
...
@patch("argparse.ArgumentParser")
@patch.object(app.Awsume, "__init__")
def test_parse_args_refresh_autocomplete(
    __init__: MagicMock,
    ArgumentParser: MagicMock,
    open: MagicMock,
    json_dump: MagicMock,
):
    __init__.return_value = None
    parser = MagicMock()
    parser.parse_args.return_value = argparse.Namespace(
        refresh_autocomplete=True, list_plugins=False
    )
    ArgumentParser.return_value = parser
    obj = app.Awsume()
    obj.config = {}
    obj.plugin_manager = MagicMock()
    obj.plugin_manager.hook.get_profile_names.return_value = [
    ...
    obj.plugin_manager.hook.get_profile_names.assert_called_with(
        config=obj.config, arguments=parser.parse_args.return_value
    )
...
def test_get_profiles(
    __init__: MagicMock, get_aws_files: MagicMock, aggregate_profiles: MagicMock
):
    __init__.return_value = None
...
def test_get_credentials(
    __init__: MagicMock, isatty: MagicMock, get_role_chain: MagicMock
):
    __init__.return_value = None
    ...
    obj.plugin_manager.hook.get_credentials.return_value = [
        {
            "AccessKeyId": "AKIA...",
...
@patch.object(sys.stdin, "isatty")
@patch.object(app.Awsume, "__init__")
def test_get_saml_credentials(
    __init__: MagicMock,
    isatty: MagicMock,
    get_role_chain: MagicMock,
    assume_role_with_saml: MagicMock,
...
@patch.object(app, "safe_print")
@patch.object(app.Awsume, "__init__")
def test_get_credentials_profile_not_found_error(
    __init__: MagicMock,
    safe_print: MagicMock,
    isatty: MagicMock,
    get_role_chain: MagicMock,
    ...
    obj.plugin_manager.hook.get_credentials.side_effect = (
...
@patch.object(app, "safe_print")
@patch.object(app.Awsume, "__init__")
def test_get_credentials_invalid_profile_error(
    __init__: MagicMock,
    safe_print: MagicMock,
    isatty: MagicMock,
    get_role_chain: MagicMock,
    ...
    obj.plugin_manager.hook.get_credentials.side_effect = (
        exceptions.InvalidProfileError(profile_name="profile")
    )
    with pytest.raises(exceptions.InvalidProfileError):
        obj.get_credentials(args, profiles)
    obj.plugin_manager.hook.catch_invalid_profile_exception.assert_called_with(
        config=obj.config,
        arguments=args,
        error=obj.plugin_manager.hook.get_credentials.side_effect,
        profiles=profiles,
...
@patch.object(app, "safe_print")
@patch.object(app.Awsume, "__init__")
def test_get_credentials_user_authentication_error(
    __init__: MagicMock,
    safe_print: MagicMock,
    isatty: MagicMock,
    get_role_chain: MagicMock,
    ...
    obj.plugin_manager.hook.get_credentials.side_effect = (
        exceptions.UserAuthenticationError()
    )
...
@patch.object(app, "safe_print")
@patch.object(app.Awsume, "__init__")
def test_get_credentials_role_authentication_error(
    __init__: MagicMock,
    safe_print: MagicMock,
    isatty: MagicMock,
    get_role_chain: MagicMock,
    ...
    obj.plugin_manager.hook.get_credentials.side_effect = (
        exceptions.RoleAuthenticationError()
    )
...
def test_export_data(
    __init__: MagicMock, stdout: MagicMock, is_mutable_profile: MagicMock
):
    __init__.return_value = None
    obj = app.Awsume()
...
def test_run(__init__: MagicMock, isatty: MagicMock):
    ...
    obj.parse_args.return_value = argparse.Namespace(
        with_saml=False,
        with_web_identity=False,
        auto_refresh=False,
    ...
    obj.export_data.assert_called_with(
        obj.parse_args.return_value,
        obj.get_profiles.return_value,
        obj.get_credentials.return_value,
        "Awsume",
        [
            "AKIA...",
